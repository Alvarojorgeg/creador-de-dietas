        ...
        if ($accion === 'datos_cuenta') {
            ...
            if ($nombre === '' || $email === '') $error = 'Nombre y email son obligatorios.';
                ...
                if ($stmt->execute()) {
                    ...
                    log_admin($conn, $uid, 'EDITAR_PROPIO', 'Datos personales actualizados');
        ...
        elseif ($accion === 'password') {
            ...
            elseif ($nueva !== $rep)                             $error = 'Las contraseñas no coinciden.';
                ...
                if (!password_verify($actual, $h)) $error = 'Contraseña actual incorrecta.';
                    ...
                    log_admin($conn, $uid, 'CAMBIO_PASS_PROPIA', 'Cambio de contraseña propia');
...
$nUsuarios   = (int)$conn->query("SELECT COUNT(*) c FROM usuarios")->fetch_assoc()['c'];
$nAdmins     = (int)$conn->query("SELECT COUNT(*) c FROM usuarios WHERE rol='admin' AND activo=1")->fetch_assoc()['c'];
$nLogsHoy    = (int)$conn->query("SELECT COUNT(*) c FROM logs_admin WHERE DATE(fecha_hora)=CURDATE()")->fetch_assoc()['c'];
$miActividad = (int)$conn->query("SELECT COUNT(*) c FROM logs_admin WHERE id_usuario_accion=$uid AND fecha_hora >= DATE_SUB(NOW(), INTERVAL 30 DAY)")->fetch_assoc()['c'];
...
<main class="page">
  ...
  <article class="card ficha-id">
    ...
    <div class="ficha-id-info">
      ...
      <div class="text-muted ficha-meta">
        Administrador desde <?= e(date('d/m/Y', strtotime($u['fecha_registro']))) ?>
      </div>
    </div>
  </article>
  ...
  <section class="card">
    ...
    <div class="prog-stats">
      <div class="prog-stat">
        <span class="prog-stat-num"><?= $nUsuarios ?></span>
        <span class="prog-stat-lbl">Usuarios sistema</span>
      </div>
      <div class="prog-stat">
        <span class="prog-stat-num"><?= $nAdmins ?></span>
        <span class="prog-stat-lbl">Admins activos</span>
      </div>
      <div class="prog-stat">
        <span class="prog-stat-num"><?= $nLogsHoy ?></span>
        <span class="prog-stat-lbl">Acciones admin hoy</span>
      </div>
      <div class="prog-stat">
        <span class="prog-stat-num"><?= $miActividad ?></span>
        <span class="prog-stat-lbl">Mis acciones (30d)</span>
      </div>
    </div>
  </section>
