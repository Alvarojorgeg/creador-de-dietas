...
// Cargar la dieta (debe ser del dietista)
$stmt = $conn->prepare(
    "SELECT d.id, d.nombre, d.icono, d.kcal_objetivo, d.prot_objetivo, d.carb_objetivo, d.grasas_objetivo,
            d.id_cliente,
            u.nombre_completo AS dietista_nombre,
            c.nombre_completo AS cliente_nombre
     FROM dietas_base d
     JOIN usuarios u ON u.id = d.id_dietista
     LEFT JOIN usuarios c ON c.id = d.id_cliente
     WHERE d.id = ? AND d.id_dietista = ?
     LIMIT 1"
);
$stmt->bind_param('ii', $idDieta, $uid);
$stmt->execute();
$dieta = $stmt->get_result()->fetch_assoc();
$stmt->close();
if (!$dieta) { header('Location: dietista_dietas.php'); exit; }

$nombreCliente = $dieta['cliente_nombre'] ?? '— Plantilla —';
...
<title>Imprimir dieta · <?= e($dieta['nombre']) ?></title>
<style>
html, body {
...
.print-actions { display: flex; gap: 10px; justify-content: center; margin-bottom: 16px; }
.btn-print, .btn-back { background: #fff; border: 1.5px solid #000; color: #000; padding: 8px 18px; font-family: inherit; font-size: 13px; border-radius: 4px; cursor: pointer; text-decoration: none; }
.btn-print:hover, .btn-back:hover { background: #f0f0f0; }
...
<div class="print-actions">
  ...
  <a class="btn-back" href="dietista_dietas.php?id=<?= (int)$dieta['id'] ?>">← Volver</a>
</div>

<div class="print-header">
  <h1 class="print-title">Dieta - <?= e($dieta['nombre']) ?></h1>
  <div class="print-meta"><strong>Cliente:</strong> <?= e($nombreCliente) ?></div>
  <div class="print-meta"><strong>Dietista:</strong> <?= e($dieta['dietista_nombre']) ?></div>
